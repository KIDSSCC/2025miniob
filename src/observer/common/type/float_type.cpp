...
int FloatType::compare(const Value &left, const Value &right) const
{
  ...
  ASSERT(right.attr_type() == AttrType::INTS || right.attr_type() == AttrType::FLOATS, "right type is not numeric but %s", attr_type_to_string(right.attr_type()));
...
RC FloatType::divide(const Value &left, const Value &right, Value &result) const
{
  ...
  LOG_DEBUG("left value is %f, right value is %f, result is %f", left.get_float(), right.get_float(), result.get_float());
  return RC::SUCCESS;
}
