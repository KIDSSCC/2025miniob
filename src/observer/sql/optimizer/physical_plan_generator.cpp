...
RC PhysicalPlanGenerator::create_plan(PredicateLogicalOperator &pred_oper, unique_ptr<PhysicalOperator> &oper, Session* session)
{
  ...
  ASSERT(children_opers.size() >= 1, "predicate logical operator's sub oper number should greater than 1");
...
RC PhysicalPlanGenerator::create_plan(ProjectCacheLogicalOperator &project_oper, unique_ptr<PhysicalOperator> &oper, Session* session)
{
  vector<unique_ptr<LogicalOperator>> &child_opers = project_oper.children();
  ...
  // 作为子查询，其字段应该是只有一个
  auto project_operator = make_unique<ProjectCachePhysicalOperator>(std::move(project_oper.expressions()), project_oper.is_relevant(), project_oper.is_check());
  if (child_phy_oper) {
    project_operator->add_child(std::move(child_phy_oper));
  }
...
RC PhysicalPlanGenerator::create_plan(InsertLogicalOperator &insert_oper, unique_ptr<PhysicalOperator> &oper, Session* session)
{
  ...
  vector<AttrInfoSqlNode>& attr_infos     = insert_oper.attr_infos();
  InsertPhysicalOperator *insert_phy_oper = new InsertPhysicalOperator(table, std::move(values));
  insert_phy_oper->attr_infos_ = std::move(attr_infos);
...
RC PhysicalPlanGenerator::create_plan(OrderByLogicalOperator &logical_oper, unique_ptr<PhysicalOperator> &oper, Session* session){
  ...
  // 创建orderby物理算子
  order_by_oper = make_unique<OrderByPhysicalOperator>(std::move(logical_oper.expressions()), std::move(logical_oper.order()));

  // OrderByLogicalOperator应该只有一个子节点
